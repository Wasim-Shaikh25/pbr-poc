...
Modes (X/Y family — used as *candidates* by the selective encoder):
...
Selection among family modes is min payload bytes; ties prefer MATRIX.
The product container does **not** store every tile as X/Y: packed-K of
the true (unpadded) nodes is the default. See ``pbr_q4.selective``.

``encode_array_xy`` (all-tile) is a comparison helper, not the product path.
"""
...
def encode_array_xy(kept: np.ndarray, nbits: int, *, th: int = TILE, tw: int = TILE) -> dict[str, Any]:
    """All-tile X/Y encode (PR #15 comparison helper). Not the product path.

    Product containers use :func:`pbr_q4.selective.encode_array_selective`.
    """
