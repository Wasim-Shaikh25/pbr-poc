"""H95X v2 container: H95Q-S1 quantized reference + selective X/Y mantissas.

Physical layout matches H95Q (magic ``H95X``, version 2)::
...
* mantissa: packed-K of true nodes, *or* a strictly smaller selective-XY
  blob (mode map + packed leftover + XY flags/payloads). Discriminated by
  blob length vs ``packed_bytes_for_k(n, K)``. No 16×16 padding.
* embed tiers: int8 row-keep table + per-K mantissa blobs
* optional raw-u16 exception words (NaN payloads / dirty lower bits)

Product default is packed-K. Always-on all-tile X/Y (PR #15) is not used.
"""
...
# Same width as H95Q's "kbit_msb_first" so a packed-only file does not inflate the header.
MANTISSA_PACKING = "kbit_or_xysel"
    ...
    *,
    ...
    if row_keeps is not None:
        ...
        for k, g in fields["groups"].items():
            enc = encode_array_selective(g["kept"], int(g["k"]))
        ...
        return {
            ...
            "mant_blob_bytes": blob_bytes,
            "map_bytes": map_bytes,
            "flag_bytes": flag_bytes,
            "xy_payload_bytes": xy_payload,
            "n_xy_sel_groups": n_xy_sel_groups,
        }
    ...
    enc = encode_array_selective(kept2, int(keep))
    return {
        "shape": fields["shape"],
        "n_weights": int(w.size),
        "mode": "uniform",
        ...
        "n_tiles": enc["n_tiles"],
        "n_xy_tiles": enc["n_xy_tiles"],
        "n_packed_tiles": enc["n_packed_tiles"],
        "packed_whole_bytes": enc["packed_whole_bytes"],
        "mant_blob_bytes": enc["blob_bytes"],
        "map_bytes": enc["map_bytes"],
        "flag_bytes": enc["flag_bytes"],
        "xy_payload_bytes": enc["xy_payload_bytes"],
        "n_xy_sel_groups": 1 if enc["kind"] == "xy_sel" else 0,
    }
...
def _decode_one(spec: dict[str, Any], data: memoryview) -> np.ndarray:
    ...
    if spec["mode"] == "uniform":
        ...
        blob = bytes(data[spec["mant_off"] : spec["mant_off"] + spec["mant_len"]])
        if len(shape) == 1:
            rows, cols = 1, n
        elif len(shape) == 2:
            rows, cols = int(shape[0]), int(shape[1])
        else:
            rows, cols = int(shape[0]), n // int(shape[0])
        kept2 = decode_array_selective(blob, rows=rows, cols=cols, nbits=k, n_nodes=n)
        kept = kept2.ravel()[:n]
    ...
    if spec["mode"] == "embed_tiers":
        ...
        for g in spec["mant_groups"]:
            ...
            kept = decode_array_selective(
                blob,
                rows=int(g["rows"]),
                cols=int(g["cols"]),
                nbits=int(g["k"]),
                n_nodes=int(g["n_weights"]),
            )
    ...
    *,
    ...
    for i, (name, w) in enumerate(unique, 1):
        ...
        if i == 1 or i == n_unique or i % 20 == 0:
            print(
                f"  xy-sel {i}/{n_unique} {name} tiles={enc.get('n_tiles', 0)} "
                f"xy={enc.get('n_xy_tiles', 0)} kind_groups={enc.get('n_xy_sel_groups', 0)}",
                flush=True,
            )
    ...
    for enc in encoded:
        ...
        packed_whole_total += int(enc.get("packed_whole_bytes") or 0)
        mant_blob_total += int(enc.get("mant_blob_bytes") or 0)
        n_tiles_total += int(enc.get("n_tiles") or 0)
        n_xy_total += int(enc.get("n_xy_tiles") or 0)
        n_packed_total += int(enc.get("n_packed_tiles") or 0)
        map_bytes_total += int(enc.get("map_bytes") or 0)
        flag_bytes_total += int(enc.get("flag_bytes") or 0)
        xy_payload_total += int(enc.get("xy_payload_bytes") or 0)
        if int(enc.get("n_xy_sel_groups") or 0):
            n_xy_sel_tensors += 1
    ...
    def build_header_json(payload_start: int) -> bytes:
        ...
        header_obj = {
            ...
            "mantissa_packing": MANTISSA_PACKING,
            "tile": [TILE, TILE],
            "xy_default": "packed",
            "hard_override": "selective_xy_only_if_strictly_smaller",
            "packed_k_is_product_default": True,
    ...
    packed_bpw = (packed_whole_total * 8.0 / total_weights) if total_weights else 0.0
    mant_bpw = (mant_blob_total * 8.0 / total_weights) if total_weights else 0.0
    ...
    return {
        ...
        "n_xy_tiles": n_xy_total,
        "n_packed_tiles": n_packed_total,
        "n_xy_sel_tensors": n_xy_sel_tensors,
        ...
        "xy_flag_bytes": flag_bytes_total,
        "xy_map_bytes": map_bytes_total,
        "xy_payload_bytes": xy_payload_total,
        "mant_blob_bytes": mant_blob_total,
        "mant_blob_bpw": round(mant_bpw, 6),
        "packed_whole_bytes": packed_whole_total,
        "packed_whole_bpw": round(packed_bpw, 6),
        "saved_vs_packed_mant_bytes": packed_whole_total - mant_blob_total,
        "xy_mode_histogram": mode_hist,
        "xy_pred_histogram": pred_hist,
        "xy_trav_histogram": trav_hist,
        "product_default_packed": True,
        "all_tiles_matrix_family": False,
        "candidate": candidate,
    }
...
def decode_container(path: str | Path) -> dict[str, Any]:
    ...
    for i, spec in enumerate(header["tensors"], 1):
        if i == 1 or i == n_spec or i % 40 == 0:
            print(f"  xy-sel-decode {i}/{n_spec} {spec['name']}", flush=True)
...
def main_decode_cli(argv: list[str] | None = None) -> int:
    ...
    p = argparse.ArgumentParser(description="H95X v2 (S1 selective X/Y) decode-only tool")
    ...
    print(f"xy_default={h.get('xy_default')}")
